...
#	- Various bug fixes
# 2012-08-29:
#	- Mail notification removed
# 2012-12-05:
# 	  Major rework and enhancement of the script:
#	- Script renamed into manageSleep.sh (before: safeShutdown.sh)
#	- The script should not be scheduled by cron anymore
#	  but called once at NAS startup
#	- Script set the NAS to sleep if no other devices are online
#	  during more than a certain duration, or if a configurable 
#	  time is reached
# 2012-12-08:
#	- Logging of system state added (this replaces the scripts run after startup / before shutdown)
#	- "always-on" timeslot added
#	- Comments improved
# 2012-12-09:
#	- Bug fix in function "isInTimeSlot"
#	- Bug fix in check if any other device is online (bad computation of duration
#	  since the last device went offline)
# 2012-12-10:
#	- It is now possible to pass the necessary params as argument of the script
# 	- Log the selected configuration when the script starts	 
# 	- Send mail in case an error occurs	 
# 2012-12-10:
#	- Support for ACPI state 3 (Suspend to RAM) added
#	- Bug fix: Lock not created anymore at script startup
# 2012-12-11:
#	- New option -v: verbose
#	- New option -m: send mail on acpi state change
#	- Bug fix: script locks deleted at startup
# 2012-12-11:
#	- Path to ACPI conf corrected
#	- support of ACPI state 3 added
#	- minor bugfix in parsing of arguments
#	- minor bugfixes in logging
# 2012-12-14:
#	- Bug fix: Scripts are now allowed to start if the NAS shall not sleep anymore
#	  (e.g. nas request sleep, but cannot because a script is running, and 
#	  before and of script execution, another device starts, so that the nas
#	  should not sleep anymore)
#	- Monitor if other devices are online, even during the initial phase
#	  during which the NAS shall stay awake
# 2012-12-17:
#       - Variable TMPFILE renamed in LOGFILE
#	- constants defining path to utilities (like date, rm...) moved to config.sh
# 2013-01-06:
#       - More compact coding for verbose logging
# 2013-01-08:
#       - minor changes in declaration of local variables
# 2013-01-13:
#       - few variables declared read only
##################################
