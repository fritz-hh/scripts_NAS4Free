...
#
# 2010-03-07:
#	Script completely rewritten in order to be more robust
#	in case the computer is not always switched on.
#	Instead of having to be called daily, weekly and monthly
#	by cron, it is now only necessary to call the script daily.
# 2010-03-08:
#	Now using an external script for sorting the snapshots
...
# 2010-12-30:
#	- Script adapted to FreeNAS 0.7.2
#	- Logging modified to harmonize the format of the log file
# 2010-12-31:
#	- Send a mail in case an issue is detected 
#	- Function aimed at deleting old snapshots added to the script
#	  (This fucntion was in a separate file before)
# 2011-01-01:
#	- Hourly tag added 
# 2011-01-02:
#	- Bug fix in "main" function 
# 2011-10-09:
#	- Function deleteOldSnapshots modified so that it does not delete
#	  any snapshot if the number of snapshots to be kept is lower than 0
#	- Function renameSnapshots modified so that it renames a snapshot if
#	  the age difference between the high frequency snapshot and low 
#	  frequency snapshot is greater or equal (and not strictly greater)
#	  than the period given as paramter
#	- Additional parameters supported by the script:
#	  [-n] [-h num] [-d num] [-w num] [-m num] 	
# 2011-10-21:
#	- Pool import added at the beginning of the script
#	- Pool export added at the beginning of the script
#       - Script modified in order to mail only the lines
#         that have been written in the logfile during the current execution
# 2011-10-22:
#	- Name of the log file modified in order to contain the name of the 
#	  filesystem for which snapshots shall be created
#       - Minor changes to improve code quality
# 2012-08-21:
#	- Minor fix in logged data (the number of snapshot kept was erroneous in the log)
...
# 2013-01-01:
#       - snapshot never renamed anymore (because it could lead to issues
#	  with the backup function)
#	- performance improved
# 2013-01-02:
#       - reference to obsolete script commonPoolFct.sh removed
#	- bug fix in snapshot creation function. Never create a snapshot if the max
#	  number of snapshots to be created is 0
# 2013-01-03:
#       - new snapshot naming rules to be compatible with NAS4Free
#	  CIFS shadow copies function
# 2013-01-04:
#       - improvements in code quality (no functionnal impact)
